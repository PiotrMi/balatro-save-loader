// from https://github.com/EFHIII/balatro-calculator/blob/main/cards.js
...
const suitNames = [heartsc, clubsc, diamondsc, spadesc];

const toggleJokerc =
  "<br>${shadowc}(Currently ${jokerValue ? `${probc}active${endc}` : `${multc}inactive${endc}`})${endc}";
...
const jokerTexts = [
  [
    ...
    [
      "Chaos the Clown",
      "${numc}1${endc} free ${probc}Reroll${endc}<br>per shop",
    ],
    [
      "Jolly Joker",
      "${multc}+8${endc} Mult if played<br>hand contains<br>a ${numc}Pair${endc}",
    ],
    [
      "Zany Joker",
      "${multc}+12${endc} Mult if played<br>hand contains<br>a ${numc}Three of a Kind${endc}",
    ],
    [
      "Mad Joker",
      "${multc}+20${endc} Mult if played<br>hand contains<br>a ${numc}Four of a Kind${endc}",
    ],
    [
      "Crazy Joker",
      "${multc}+12${endc} Mult if played<br>hand contains<br>a ${numc}Straight${endc}",
    ],
    [
      "Droll Joker",
      "${multc}+10${endc} Mult if played<br>hand contains<br>a ${numc}Flush${endc}",
    ],

    [
      "Half Joker",
      "${multc}+20${endc} Mult if played<br>hand contains<br>${numc}3${endc} or fewer cards",
    ],
    [
      "Merry Andy",
      "${multc}+3${endc} discards,<br>${multc}-1${endc} hand size",
    ],
    [
      "Stone Joker",
      "This Joker gains ${chipc}+25${endc} Chips<br>for each ${numc}Stone Card${endc} in your full deck<br>${shadowc}(Currently ${chipc}+${25*jokerValue}${endc} Chips)${endc}",
    ],
  ],
  [
    ...
    [
      "Acrobat",
      "${prodc}3${endc} Mult on ${numc}final<br>hand${endc} of round" +
        toggleJokerc,
    ],
    ...
    [
      "Greedy Joker",
      "Played cards with<br>${diamondc} suit give<br>${multc}+4${endc} Mult when scored",
    ],
    [
      "Lusty Joker",
      "Played cards with<br>${heartc} suit give<br>${multc}+4${endc} Mult when scored",
    ],
    [
      "Wrathful Joker",
      "Played cards with<br>${spadec} suit give<br>${multc}+4${endc} Mult when scored",
    ],
    [
      "Gluttonous Joker",
      "Played cards with<br>${clubc} suit give<br>${multc}+4${endc} Mult when scored",
    ],
  ],
  [
    [
      "Troubadour",
      "${numc}+2${endc} hand size,<br>${multc}-1${endc} hands per round",
    ],
    [
      "Banner",
      "${chipc}+40${endc} Chips for<br>each remaining<br>${numc}discard${endc}<br>${shadowc}(Currently ${chipc}+${jokerValue * 40}${endc} Chips)${endc}",
    ],
    [
      "Mystic Summit",
      "${multc}+15${endc} Mult when<br>${numc}0${endc} discards<br>remaining" +
        toggleJokerc,
    ],
    [
      "Marble Joker",
      "Adds one ${numc}Stone${endc} card<br>to deck when<br>${numc}Blind${endc} is selected",
    ],
    [
      "Loyalty Card",
      "${prodc}4${endc} Mult every<br>${numc}6${endc} hands played<br>${shadowc}(${jokerValue % 6} remaining)${endc}",
    ],
    [
      "Hack",
      "Retrigger<br>each played<br>${numc}2${endc}, ${numc}3${endc}, ${numc}4${endc}, or ${numc}5${endc}",
    ],
    ["Misprint", "${multc}+0${endc} - ${multc}+23${endc} Mult"],
    [
      "Steel Joker",
      "This Joker gains ${prodc}0.25${endc} Mult<br>for each ${numc}Steel Card${endc}<br>in your full deck<br>${shadowc}(Currently ${prodc}${1 + jokerValue * 0.25}${endc} Mult)${endc}",
    ],
    [
      "Raised Fist",
      "Adds ${numc}double${endc} the rank<br>of ${numc}lowest${endc} card held in hand to Mult",
    ],
  ...
  [
    ...
    [
      "Glass Joker",
      "Gains ${prodc}0.5${endc} Mult<br>for every ${numc}Glass Card${endc}<br>that is destroyed<br>${shadowc}(Currently ${prodc}${1 + jokerValue * 0.5}${endc} Mult)${endc}",
    ],
    [
      "Scary Face",
      "Played ${numc}face${endc} cards<br>give ${chipc}+30${endc} Chips<br>when scored",
    ],
    [
      "Abstract Joker",
      "${multc}+3${endc} Mult for<br>each ${numc}Joker${endc} card<br>${shadowc}(Currently ${multc}+${jokerValue * 3}${endc} Mult)${endc}",
    ],
    [
      "Delayed Gratification",
      "Earn ${moneyc}$2${endc} per ${numc}discard${endc} if<br>no discards are used<br>by end of the round",
    ],
    [
      "Golden Ticket",
      "Played ${moneyc}Gold${endc} cards<br>earn ${moneyc}$3${endc} when scored",
    ],
    ["Pareidolia", "All cards are<br>considered<br>${numc}Face${endc} cards"],
    [
      "Cartomancer",
      "Create a ${tarotc} card<br>when ${numc}Blind${endc} is selected${roomc}",
    ],
    [
      "Even Steven",
      "Played cards with<br>${numc}even${endc} rank give<br>${multc}+4${endc} Mult when scored<br>${shadowc}(10, 8, 6, 4, 2)${endc}",
    ],
    [
      "Odd Todd",
      "Played cards with<br>${numc}odd${endc} rank give<br>${chipc}+30${endc} Chips when scored<br>${shadowc}(A, 9, 7, 5, 3)${endc}",
    ],
  ],
  [
    [
      "Wee Joker",
      "This Joker gains<br>${chipc}+8${endc} Chips when each<br>played ${numc}2${endc} is scored<br>${shadowc}(Currently ${chipc}+${10 + jokerValue * 8}${endc} Chips)${endc}",
    ],
    [
      "Business Card",
      "Played ${numc}Face${endc} cards have<br>a ${probc}1 in 2${endc} chance to<br>give ${moneyc}$2${endc} when scored",
    ],
    [
      "Supernova",
      "Adds the number of<br>times ${numc}poker hand${endc} has<br>been played to Mult<br>${shadowc}(Currently ${multc}+${jokerValue}${endc} Mult)${endc}",
    ],
    [
      "Mr. Bones",
      "Prevents Death<br>if chips scored<br>are at least ${numc}25%${endc}<br>of required chips<br>${multc}self destructs${endc}",
    ],
    [
      "Seeing Double",
      "${prodc}2${endc} Mult if played<br>hand has a scoring<br>Club card and a scoring<br>card of any other ${numc}suit${endc}",
    ],
    [
      "The Duo",
      "${prodc}2${endc} Mult if played<br>hand contains<br>a ${numc}Pair${endc}",
    ],
    [
      "The Trio",
      "${prodc}3${endc} Mult if played<br>hand contains<br>a ${numc}Three of a Kind${endc}",
    ],
    [
      "The Family",
      "${prodc}4${endc} Mult if played<br>hand contains<br>a ${numc}Four of a Kind${endc}",
    ],
    [
      "The Order",
      "${prodc}3${endc} Mult if played<br>hand contains<br>a ${numc}Straight${endc}",
    ],
    [
      "The Tribe",
      "${prodc}2${endc} Mult if played<br>hand contains<br>a ${numc}Flush${endc}",
    ],
  ],
  [
    [
      "8 Ball",
      "Create a ${planetc} card<br>if played hand contains<br>2 or more ${numc}8s${roomc}",
    ],
    [
      "Fibonacci",
      "Each plaed ${numc}Ace${endc},<br>${numc}2${endc}, ${numc}3${endc}, ${numc}5${endc}, or ${numc}8${endc} gives<br>${multc}+8${endc} Mult when scored",
    ],
    [
      "Joker Stencil",
      "${prodc}1${endc} Mult for each<br>empty ${numc}Joker${endc} slot<br>Joker stencil included<br>${shadowc}(Currently ${prodc}${1 + jokerValue}${endc})${endc}",
    ],
    [
      "Space Joker",
      "${probc}1 in 4${endc} chane to<br>upgrade level of<br>played ${numc}poker hand${endc}",
    ],
    [
      "Matador",
      "Earn ${moneyc}$8${endc} if played<br>hand triggers the<br>${numc}Boss Blind${endc} ability",
    ],
    [
      "Ceremonial Dagger",
      "When ${numc}Blind${endc} is selected,<br>destroy Joker to the right<br>and permanently add ${numc}double${endc}<br>its sell value to this ${multc}Mult${endc}<br>${shadowc}(Currently ${multc}+${jokerValue * 2}${endc} Mult)${endc}",
    ],
    [
      "Showman",
      "${numc}Joker${endc}, ${tarotc}, ${planetc},<br>and ${spectralc} cards may<br>appear multiple times",
    ],
    [
      "Fortune Teller",
      "${multc}+1${endc} Mult per ${tarotc}<br>card used this run<br>${shadowc}(Currently ${multc}${jokerValue}${endc})${endc}",
    ],
    [
      "Hit the Road",
      "Gains ${prodc}0.5${endc} Mult<br>per discarded<br>${numc}Jack${endc} this round<br>${shadowc}(Currently ${prodc}${1 + jokerValue * 0.5}${endc})${endc}",
    ],
    [
      "Swashbuckler",
      "Adds the sell value of<br>all owned ${numc}Jokers${endc} left<br>of this card to Mult",
    ],
  ],
  [
    [
      "Flower Pot",
      "${prodc}3${endc} Mult if played<br>hand has a scoring<br>${diamondc} card, ${clubc} card,<br>${heartc} card, and ${spadec} card",
    ],
    [
      "Ride the Bus",
      "${multc}+1${endc} Mult per<br>consecutive hand<br>played without a<br>scoring ${numc}face${endc} card<br>${shadowc}(Currently ${multc}+${jokerValue}${endc})${endc}",
    ],
    [
      "Shoot the Moon",
      "${multc}+13${endc} Mult for<br>each ${numc}Queen${endc}<br>held in hand",
    ],
    [
      "Scholar",
      "Played ${numc}Aces${endc}<br>give ${chipc}+20${endc} Chips<br>and ${multc}+4${endc} Mult<br>when scored",
    ],
    [
      "Smeared Joker",
      "${heartc} and ${diamondc}<br>count as the same suit,<br>${spadec} and ${clubc}<br>count as the same suit",
    ],
    [
      "Oops! All 6s",
      "Double all ${numc}listed${endc}<br>${probc}probabilities${endc}<br>${shadowc}(ex: ${probc}1 in 3${endc} -> ${probc}2 in 3${endc})${endc}",
    ],
    [
      "Four Fingers",
      "All ${numc}Flushes${endc} and<br>${numc}Straights${endc} can be<br>made with ${numc}4${endc} cards",
    ],
    [
      "Gros Michel",
      "${multc}+15${endc} Mult<br>${probc}1 in 4${endc} chance this card is destroyed<br>at the end of the round",
    ],
    ["Stuntman", "${chipc}+300${endc} Chips,<br>${numc}-2${endc} hand size"],
    [
      "Hanging Chad",
      "Retrigger ${numc}first${endc} played<br>card used in scoring",
    ],
  ],
  [
    [
      "Driver's License",
      "${prodc}3${endc} Mult if you have<br>at least ${numc}16${endc} Enhanced<br>cards in your full deck<br>${shadowc}(Currently ${numc}${jokerValue}${endc})${endc}",
    ],
    [
      "Invisible Joker",
      "After 3 rounds,<br>sell this card to<br>${numc}duplicate${endc} a random Joker<br>${shadowc}(Currently ${numc}${jokerValue}${endc}/3)${endc}",
    ],
    [
      "Astronomer",
      "All ${planetc} cards and<br>${celestialc} in<br>the shop are ${numc}free${endc}",
    ],
    [
      "Burnt Joker",
      "Upgrade the level of<br>the first ${numc}discarded${endc}<br>poker hand each round",
    ],
    [
      "Dusk",
      "Retrigger all played<br>cards in ${numc}final<br>hand${endc} of round" +
        toggleJokerc,
    ],
    [
      "Throwback",
      "${prodc}0.25${endc} Mult for each<br>${numc}Blind${endc} skipped this run<br>${shadowc}(Currently ${prodc}${1 + jokerValue * 0.25}${endc} Mult)${endc}",
    ],
    [
      "The Idol",
      "Each played ${numc}${rankNames[Math.floor(Math.abs(jokerValue)/4) % 13]}${endc}<br>of ${suitNames[Math.abs(jokerValue) % 4]} gives<br>${prodc}2${endc} Mult when scored<br>${shadowc}Card changes every round${endc}",
    ],
    ["Brainstorm", "Copies the ability<br>of the leftmost Joker"],
    [
      "Satellite",
      "Earn ${moneyc}$1${endc} at the end of<br>round per unique ${planetc}<br>card used this run<br>${shadowc}(Currently ${moneyc}$${jokerValue}${endc})${endc}",
    ],
    [
      "Rough Gem",
      "Played cards with<br>${diamondc} suit earn<br>${moneyc}$1${endc} when scored",
    ],
  ],
  [
    [
      "Bloodstone",
      "${probc}1 in 3${endc} chance for<br>played cards with<br>${heartc} suit to give<br>${prodc}2${endc} Mult when scored",
    ],
    [
      "Arrowhead",
      "Played cards with<br>${spadec} suit give<br>${chipc}+50${endc} Chips when scored",
    ],
    [
      "Onyx Agate",
      "Played cards with<br>${clubc} suit give<br>${multc}+8${endc} Mult when scored",
    ],
    [
      "Caino",
      "Gains ${prodc}1${endc} Mult when<br>a ${numc}face${endc} card<br>is destroyed<br>${shadowc}(Currently ${prodc}${1 + jokerValue}${endc} Mult)${endc}",
    ],
    [
      "Triboulet",
      "Played ${numc}Kings${endc} and<br>${numc}Queens${endc} each give<br>${prodc}2${endc} Mult when scored",
    ],
    [
      "Yorick",
      "${prodc}5${endc} Mult only after<br>using 23 discards<br>${shadowc}(Discards left ${numc}${jokerValue}${endc})${endc}",
    ],
    ["Chicot", "Disables effect of<br>every ${numc}Boss Blind${endc}"],
    [
      "Perkeo",
      "Creates a ${negativec} copy of<br>${numc}1${endc} random ${numc}consumable${endc}<br>card in your possession<br>at the end of the ${numc}shop${endc}",
    ],
    [
      "Certificate",
      "When round begins,<br>add a random ${numc}playing<br>card${endc} with a random<br>${numc}seal${endc} to your hand",
    ],
    [
      "Bootstraps",
      "${multc}+2${endc} Mult for every<br>${moneyc}$5${endc} you have<br>${shadowc}(Currently ${multc}+${jokerValue * 2}${endc} Mult)${endc}",
    ],
  ],
  [],
  [
    [
      "Egg",
      "Gains ${moneyc}$3${endc} of<br>${numc}sell value${endc} at<br>end of round",
    ],
    [
      "Burglar",
      "When ${numc}Blind${endc} is selected,<br>gain ${chipc}+3${endc} Hands and<br>${numc}lose all discards${endc}",
    ],
    [
      "Blackboard",
      "${prodc}3${endc} Mult if all<br>cards held in hand<br>are ${spadec} or ${clubc}",
    ],
    [
      "Runner",
      "Gains ${chipc}+10${endc} Chips<br>if played hand<br>contains a ${numc}Straight${endc}<br>${shadowc}(Currently ${chipc}+${20 + jokerValue * 10}${endc} Chips)${endc}",
    ],
    [
      "Ice Cream",
      '${chipc}${jokerValue > 20 ? "" : "+"}${Math.max(0, 100 - jokerValue * 5)}${endc} Chips<br>${chipc}-5${endc} Chips for<br>every hand played',
    ],
    [
      "DNA",
      "If ${numc}first hand${endc} of round<br>has only ${numc}1${endc} card, add a<br>permanent copy to deck<br>and draw it to ${numc}hand${endc}",
    ],
    ["Splash", "Every ${numc} played card${endc}<br>counts in scoring"],
    [
      "Blue Joker",
      "${chipc}+2${endc} Chips for each<br>remaining card in ${numc}Deck${endc}<br>${shadowc}(Currently ${chipc}+${104 + jokerValue * 2}${endc} Chips)${endc}",
    ],
    [
      "Sixth Sense",
      "If ${numc}first hand${endc} of round is<br>a single ${numc}6${endc}, destroy it and<br>create a ${spectralc} card${roomc}",
    ],
    [
      "Constellation",
      "Gains ${prodc}0.1${endc} Mult<br>per ${planetc} card used<br>${shadowc}(Currently ${prodc}${(Math.max(0, 10 + jokerValue))/10}${endc} Mult)${endc}",
    ],
  ],
  [
    [
      "Hiker",
      "Every played ${numc}card${endc}<br>permanently gains<br>${chipc}+4${endc} Chips when scored",
    ],
    [
      "Faceless Joker",
      "Earn ${moneyc}$5${endc} if ${numc}3${endc} or<br>more ${numc}face cards${endc}<br>are discarded<br>at the same time",
    ],
    [
      "Green Joker",
      '${multc}+1${endc} Mult per hand played<br>${multc}-1${endc} Mult per discard<br>${shadowc}(Currently ${multc}${jokerValue >= 0 ? "+" : ""}${jokerValue}${endc} Mult)${endc}',
    ],
    [
      "Superposition",
      "Create a ${tarotc} card if<br>poker hand contains an<br>${numc}Ace${endc} and a ${numc}Straight${endc}${roomc}",
    ],
    [
      "To Do List",
      "Earn ${moneyc}$5${endc} if ${numc}poker hand${endc}<br>is a ${numc}Pair${endc},<br>poker hand changes<br>on every payout",
    ],
    [
      "Cavendish",
      "${prodc}3${endc} Mult<br>${probc}1 in 1000${endc} chance this<br>card is destroy<br>at end of round",
    ],
    [
      "Card Sharp",
      "${prodc}3${endc} Mult if played<br>${numc}poker hand${endc} has already<br>been played this round" +
        toggleJokerc,
    ],
    [
      "Red Card",
      "Gains ${multc}+3${endc} Mult when any<br>${numc}Booster Pack${endc} is skipped<br>${shadowc}(Currently ${multc}+${jokerValue * 3}${endc} Mult)${endc}",
    ],
    [
      "Madness",
      "When ${numc}Blind${endc} is selected,<br>gain ${prodc}0.5${endc} Mult and<br>${numc}destroy${endc} a random Joker<br>${shadowc}(Currently ${prodc}${1 + jokerValue * 0.5}${endc})${endc}",
    ],
    [
      "Square Joker",
      "Gain ${chipc}+4${endc} Chips if<br>played hand has<br>exactly ${numc}4${endc} card<br>${shadowc}(Currently ${chipc}+${16 + jokerValue * 4}${endc} Chips)${endc}",
    ],
  ],
  [
    [
      "Séance",
      "If ${numc}poker hand${endc} is a<br>${numc}Straight Flush${endc}, craeate a<br>random ${spectralc} card${roomc}",
    ],
    [
      "Riff-raff",
      "When ${numc}Blind${endc} is selected,<br>create ${numc}2${endc} ${commonc} ${numc}Jokers${endc}${roomc}",
    ],
    [
      "Vampire",
      "Gains ${prodc}0.2${endc} Mult per<br>${numc}Enhanced card${endc} played,<br>removes card ${numc}Enhancement${endc}<br>${shadowc}(Currently ${prodc}${(10 + jokerValue * 2) / 10}${endc} Mult)${endc}",
    ],
    [
      "Shortcut",
      "Allows ${numc}Straights${endc} to be<br>made with gaps of ${numc}1 rank${endc}<br>${shadowc}(ex: ${numc}2 3 5 7 8${endc})${endc}",
    ],
    [
      "Hologram",
      "Gains ${prodc}0.25${endc} Mult<br> per ${numc}playing card${endc} added<br>to your deck<br>${shadowc}(Currently ${prodc}${1 + jokerValue * 0.25}${endc} Mult)${endc}",
    ],
    [
      "Vagabond",
      "Create a ${tarotc} card<br>if hand is played<br>with ${moneyc}$3${endc} or less",
    ],
    [
      "Baron",
      "Each ${numc}King${endc}<br>held in hand<br>gives ${prodc}1.5${endc} Mult",
    ],
    [
      "Cloud 9",
      "Earn ${moneyc}$1${endc} for each<br>${numc}9${endc} in your ${numc}full deck${endc}<br>at end of round<br>${shadowc}(Currently ${moneyc}$${jokerValue}${endc})${endc}",
    ],
    [
      "Rocket",
      "Earn ${moneyc}$${1 + jokerValue * 2}${endc} at end of<br>round. Gains ${moneyc}$2${endc} when<br>${numc}Boss Blind${endc} is defeated",
    ],
    [
      "Obelisk",
      "${prodc}0.2${endc} Mult per<br>consecutive hand played<br>without playing your<br>must played ${numc}poker hand${endc}<br>${shadowc}(Currently ${prodc}${(10 + jokerValue * 2) / 10}${endc} Mult)${endc}",
    ],
  ],
  [
    [
      "Midas Mask",
      "All ${numc}face${endc} cards<br>become ${numc}Gold${endc} cards<br>when played",
    ],
    [
      "Luchador",
      "Sell this card to<br>disable the current<br>${numc}Boss Blind${endc}",
    ],
    [
      "Photograph",
      "First played ${numc}face${endc}<br>card gives ${prodc}2${endc} Mult<br>when scored",
    ],
    [
      "Gift Card",
      "Add ${moneyc}$1${endc} of ${numc}sell value${endc}<br>to every ${numc}Joker${endc} and<br>${numc}Consumable${endc} card at<br>end of round",
    ],
    [
      "Turtle Bean",
      "${numc}+5${endc} hand size,<br>reduces by<br>${multc}1${endc} every round",
    ],
    [
      "Erosion",
      '${multc}+4${endc} Mult for each<br>card below ${numc}52${endc}<br>in your full deck<br>${shadowc}(Currently ${multc}${jokerValue >= 0 ? "+" : ""}${jokerValue * 4}${endc})${endc}',
    ],
    [
      "Reserved Parking",
      "Each ${numc}face${endc} card<br>held in hand has<br>a ${probc}1 in 2${endc} chance<br>to give ${moneyc}$1${endc}",
    ],
    [
      "Mail-In Rebate",
      "Earn ${moneyc}$3${endc} for each<br>discarded ${numc}${rankNames[Math.abs(jokerValue % 13)]}${endc}, rank<br>changes every round",
    ],
    [
      "To the Moon",
      "Earn an extra ${moneyc}$1${endc} of<br>${numc}interest${endc} for every ${moneyc}$5${endc} you<br>have at end of round",
    ],
    [
      "Hallucination",
      "${probc}1 in 2${endc} chance to create<br>a ${tarotc} card when any<br>${numc}Booster Pack${endc} is opened${roomc}",
    ],
  ],
  [
    [
      "Sly Joker",
      "${chipc}+50${endc} Chips if played<br>hand contains<br>a ${numc}Pair${endc}",
    ],
    [
      "Wily Joker",
      "${chipc}+100${endc} Chips if played<br>hand contains<br>a ${numc}Three of a Kind${endc}",
    ],
    [
      "Clever Joker",
      "${chipc}+150${endc} Chips if played<br>hand contains<br>a ${numc}Four of a Kind${endc}",
    ],
    [
      "Devious Joker",
      "${chipc}+100${endc} Chips if played<br>hand contains<br>a ${numc}Straight${endc}",
    ],
    [
      "Crafty Joker",
      "${chipc}+80${endc} Chips if played<br>hand contains<br>a ${numc}Flush${endc}",
    ],
    [
      "Lucky Cat",
      "Gains ${prodc}0.2${endc} Mult each<br>time a ${numc}Lucky${endc} card<br>${probc}successfully${endc} triggers<br>${shadowc}(currently ${prodc}${(10 + jokerValue * 2) / 10}${endc} Mult)${endc}",
    ],
    [
      "Baseball Card",
      "${probc}Uncommon${endc} Jokers<br>each give ${prodc}1.5${endc} Mult",
    ],
    [
      "Bull",
      "${chipc}+2${endc} Chips for each<br>${moneyc}dollar${endc} you have<br>${shadowc}(Currently ${chipc}+${Math.max(0, jokerValue * 2)}${endc} Chips)${endc}",
    ],
    [
      "Diet Cola",
      "Sell this card to<br>create a free<br>${numc}Double Tag${endc}",
    ],
    [
      "Trading Card",
      "If ${numc}first discard${endc} of round<br>has only ${numc}1${endc} card, destroy<br>it and earn ${moneyc}$3${endc}",
    ],
  ],
  [
    [
      "Flash Card",
      "${multc}+2${endc} Mult per<br>${numc}reroll${endc} in the shop<br>${shadowc}(Currently ${multc}+${jokerValue * 2}${endc} Mult)${endc}",
    ],
    [
      "Popcorn",
      '${multc}${jokerValue <= 5 ? "+" : ""}${Math.max(0, 20 - jokerValue * 4)}${endc} Mult<br>${multc}-4${endc} Mult per<br>round played',
    ],
    [
      "Ramen",
      "${prodc}${(Math.max(0, 200 - jokerValue)) / 100}${endc} Mult,<br>loses ${prodc}0.01${endc} Mult<br>per ${numc}card${endc} discarded",
    ],
    [
      "Seltzer",
      "Retrigger all<br>cards played for<br>the next ${numc}10${endc} hands",
    ],
    [
      "Spare Trousers",
      "Gain ${multc}+2${endc} Mult if<br>played hand contains<br>a ${numc}Two Pair${endc}<br>${shadowc}(Currently ${multc}+${jokerValue * 2}${endc} Mult)${endc}",
    ],
    [
      "Campfire",
      "This Joker gains ${prodc}0.5${endc} Mult<br>for each card ${numc}sold${endc}, resets<br>when ${numc}Boss Blind${endc} is defeated<br>${shadowc}(Currently ${prodc}${1 + jokerValue * 0.5}${endc} Mult)${endc}",
    ],
    [
      "Smiley Face",
      "Played ${numc}face${endc} cards<br>give ${multc}+4${endc} Mult<br>when scored",
    ],
    [
      "Ancient Joker",
      "Each played card with<br>${[heartc, clubc, diamondc, spadec][Math.abs(jokerValue) % 4]} suit gives<br>${prodc}1.5${endc} Mult when scored<br>${shadowc}suit changes at end of round${endc}",
    ],
    [
      "Walkie Talkie",
      "Each played ${numc}10${endc} or ${numc}4${endc}<br>gives ${chipc}+10${endc} Chips and<br>${multc}+4${endc} Mult when scored",
    ],
    [
      "Castle",
      "This Joker gains ${chipc}+3${endc} Chips<br>per discarded ${[heartc, clubc, diamondc, spadec][Math.abs(jokerValue) % 4]} card,<br>suit changes every round<br>${shadowc}(Currently ${chipc}+${jokerValue * 3}${endc} Chips)${endc}",
    ],
  ],
];
